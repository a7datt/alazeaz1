              ...
              <div className="space-y-3">
                ...
                {(editingItem.store_type==='quantities'||editingItem.store_type==='external_api') ? (
                  <div className="space-y-2 p-3 bg-gray-50 rounded-xl">
                    ...
                    <input type="number" placeholder="أكثر كمية" className="w-full p-2 bg-white rounded-lg text-sm outline-none" value={editingItem.max_quantity||""} onChange={e => setEditingItem({...editingItem,max_quantity:e.target.value})}/>
                    <div>
                      <p className="text-[10px] text-gray-400 font-bold mb-1">سعر التكلفة (API) $ / وحدة</p>
                      <input type="number" step="0.000001" placeholder="سعر التكلفة من API" className="w-full p-2 bg-white rounded-lg text-sm outline-none" value={editingItem.price_per_unit||""} onChange={e => setEditingItem({...editingItem,price_per_unit:e.target.value})}/>
                    </div>
                    <div>
                      <p className="text-[10px] text-green-600 font-bold mb-1">سعر البيع للعميل $ / وحدة</p>
                      <input type="number" step="0.000001" placeholder="سعر البيع (مع الربح)" className="w-full p-2 bg-white rounded-lg text-sm outline-none border border-green-200" value={editingItem.price||""} onChange={e => setEditingItem({...editingItem,price:e.target.value})}/>
                    </div>
