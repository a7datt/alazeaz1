...
```mermaid
flowchart LR
    A(["▸ تواصل معنا\nContact Us"]) -->|"Free Consultation"| B(["▸ تحليل المتطلبات\nRequirements"])
    B -->|"Custom Plan"| C(["▸ تصميم UI/UX\nDesign Phase"])
    C -->|"Your Approval"| D(["▸ التطوير\nDevelopment"])
    D -->|"Full QA Testing"| E(["▸ مراجعة الجودة\nQuality Review"])
    E -->|"Go Live"| F(["▸ إطلاق المتجر\nStore Launch"])
    F -->|"Continuous Support"| G(["▸ تحليل ونمو\nGrowth Analytics"])

    style A fill:#7c3aed,color:#fff,stroke:#7c3aed
    style C fill:#06b6d4,color:#fff,stroke:#06b6d4
